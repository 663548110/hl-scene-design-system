    ...
    <div class="color-content__content" :style="contentStyle">
      <div class="color-content__main">
        ...
        <t-row :gutter="[4, 4]" :style="{ marginLeft: '0' }">
          <t-col v-for="(color, idx) in displayColors" :key="idx" :span="3" :style="{ padding: '0' }">
            <div
              :class="{
...
import { DEFAULT_THEME_META } from '@/common/themes/built-in';
...
export default {
  ...
  computed: {
    ...
    // 主题色预设色块：第一个色块始终显示当前主题的品牌色
    displayColors() {
      const themeColor = {
        name: this.isEn ? (themeStore.theme.enName || 'Theme Color') : (themeStore.theme.name || '主题色'),
        enName: themeStore.theme.enName || 'Theme Color',
        value: themeStore.theme.value || DEFAULT_THEME_META.value,
      };
      // 如果主题色已经在 DEFAULT_COLORS 中，直接返回 DEFAULT_COLORS
      if (DEFAULT_COLORS.some(c => c.value.toLowerCase() === themeColor.value.toLowerCase())) {
        return DEFAULT_COLORS;
      }
      // 否则替换第一个为当前主题色
      return [themeColor, ...DEFAULT_COLORS.slice(1)];
    },
  },
  watch: {
    generationMode() {
