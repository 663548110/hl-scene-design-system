<template>
  <t-popup hide-empty-popup :attach="handleAttach" :visible="isThemeTabVisible" @visible-change="handleVisibleChange">
    <div
      ...
      @mousedown="dragStart"
    >
      <div class="dock__theme-tab" :style="{ overflow: 'hidden', maxHeight: isThemeTabVisible ? '400px' : '0px' }">
        <recommend-themes v-if="isThemeTabVisible" :class="{ 'fade-in': isThemeContentReady }" @add-theme="handleAddTheme" @select-theme="handleAddTheme" />
      </div>
...
export default {
  ...
  watch: {
    ...
    isThemeTabVisible(v) {
      if (v) {
        // 容器展开动画 300ms 后渐入主题列表
        setTimeout(() => { this.isThemeContentReady = true; }, 300);
      } else {
        this.isThemeContentReady = false;
      }
    },
  },
  mounted() {
  ...
  methods: {
    ...
    handleVisibleChange(visible, ctx) {
      if (!visible && ctx.trigger === 'document' && ctx.e.target?.localName !== 'td-theme-generator') {
        // 点击 Dialog 内部元素时不关闭推荐主题面板
        const target = ctx.e.target;
        if (target?.closest('.t-dialog__ctx') || target?.closest('.t-overlay')) return;
...
.dock {
  ...
  border-radius: 32px;
  &__theme-tab {
    transition: max-height 0.3s cubic-bezier(0.38, 0, 0.24, 1);
    /deep/ .recommend-theme {
      opacity: 0;
