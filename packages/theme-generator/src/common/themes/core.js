...
/**
 * 异步初始化主题样式表，支持从 API 获取自定义主题的 CSS 变量
 * - 当 themeIdOrName 以 'custom-' 开头时，从 API 获取变量
 * - 否则使用内置主题逻辑（同 initThemeStyleSheet）
 */
export async function initThemeStyleSheetAsync(themeIdOrName, device) {
  const styleSheet = appendStyleSheet(CUSTOM_THEME_ID);
  const darkStyleSheet = appendStyleSheet(CUSTOM_DARK_ID);
  const extraStyleSheet = appendStyleSheet(CUSTOM_EXTRA_ID);

  // Custom theme: fetch from API
  if (typeof themeIdOrName === 'string' && themeIdOrName.startsWith('custom-')) {
    try {
      const res = await fetch(`/api/themes/${themeIdOrName}/variables`);
      if (!res.ok) throw new Error('API error');
      const variables = await res.json();
      styleSheet.textContent = variables.light || '';
      darkStyleSheet.textContent = variables.dark || '';
      extraStyleSheet.textContent = variables.extra || '';
      return null; // caller should already have the theme object
    } catch (err) {
      console.warn('获取自定义主题变量失败:', err.message);
      return null;
    }
  }

  // Built-in theme: use existing logic
  const deviceType = normalizeDevice(device);
  const theme = findThemeByEnName(deviceType, themeIdOrName);
  const { light, dark, extra } = theme.css;
  styleSheet.textContent = light;
  darkStyleSheet.textContent = dark;
  extraStyleSheet.textContent = extra;
