...
基于微信小程序的设计，RDesign 提供了 4 种方式用于样式覆盖

## 1 使用 Style

> RDesign 全部组件均支持 `custom-style` 和 `style` 属性。可传入 CSS 字符串，将会应用于组件的根元素。

### 开启 virtualHost

从 `1.0.0-rc` 版本开始，在基础库版本高于 2.19.2 的情况下，RDesign 会默认开启 `virtualHost` 属性。
...
RDesign 全体组件均开启了 `addGlobalClass`，可以接受外部传入的样式。
...
RDesign 在每个组件的内部都预置了许多外部样式类供开发者使用。
...
RDesign 为所有组件预置了许多 CSS 变量，具体可以查看每个组件的 WXSS 文件。
