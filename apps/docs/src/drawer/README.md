...
[td_drawer_page.dart](https://codeup.aliyun.com/656d6f743e469c2f3534a9ac/team3/flutter/rdesign-flutter-develop/blob/master/rdesign-component/example/lib/page/td_drawer_page.dart)
  ...
  return RDButton(
    ...
    onTap: () {
      RDDrawer(
        ...
        items: List.generate(
            drawerItemLength, (index) => RDDrawerItem(title: '菜单${index + 1}')),
    ...
    onTap: () {
      RDDrawer(
        ...
        items: List.generate(
            drawerItemLength,
            (index) => RDDrawerItem(
                title: '菜单${index + 1}', icon: const Icon(RDIcons.app))),
      );
    },
  );
}</pre>
  ...
  return RDButton(
    ...
    onTap: () {
      RDDrawer(
        ...
        drawerTop: renderBox?.size.height,
        title: '标题',
        placement: RDDrawerPlacement.left,
        items: List.generate(
            drawerItemLength, (index) => RDDrawerItem(title: '菜单${index + 1}')),
      );
    },
...
| items | List<RDDrawerItem>? | - | 抽屉里的列表项 |
| onClose | VoidCallback? | - | 关闭时触发 |
| onItemClick | RDDrawerItemClickCallback? | - | 点击抽屉里的列表项触发 |
| placement | RDDrawerPlacement? | RDDrawerPlacement.right | 抽屉方向 |
| showOverlay | bool? | true | 是否显示遮罩层 |
| style | RDCellStyle? | - | 列表自定义样式 |
| title | String? | - | 抽屉的标题 |
