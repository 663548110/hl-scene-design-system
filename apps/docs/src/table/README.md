...
[td_table_page.dart](https://codeup.aliyun.com/656d6f743e469c2f3534a9ac/team3/flutter/rdesign-flutter-develop/blob/master/rdesign-component/example/lib/page/td_table_page.dart)
...
<td-code-block panel="Dart">
  ...
  Widget _basicTable(BuildContext context) {
    return RDTable(
      columns: [
        RDTableCol(title: '标题', colKey: 'title1', ellipsis: true),
        RDTableCol(title: '标题', colKey: 'title2'),
        RDTableCol(title: '标题', colKey: 'title3'),
        RDTableCol(title: '标题', colKey: 'title4')
      ],
      data: _getData(9),
    );
  }</pre>
  ...
  Widget _sortableTable(BuildContext context) {
    return RDTable(
      columns: [
        ...
        RDTableCol(title: '标题', colKey: 'title2', sortable: true),
        RDTableCol(title: '标题', colKey: 'title3', sortable: true),
        RDTableCol(title: '标题', colKey: 'title4', sortable: true)
      ],
      data: _getData(9),
    );
  }</pre>
        ...
        RDTableCol(
          ...
          cellBuilder: (BuildContext context, int index) {
            return Row(
              ...
              children: [
                Icon(RDIcons.upload,
                    color: RDTheme.of(context).brandNormalColor, size: 16),
                Icon(RDIcons.delete,
                    color: RDTheme.of(context).brandNormalColor, size: 16),
              ],
            );
...
<td-code-block panel="Dart">
  ...
  Widget _fixedFirstColTable(BuildContext context) {
    return RDTable(
      columns: [
        RDTableCol(title: '标题', colKey: 'title1'),
        RDTableCol(title: '标题', colKey: 'title2'),
        RDTableCol(title: '标题', colKey: 'title3'),
        RDTableCol(title: '标题', colKey: 'title4', fixed: RDTableColFixed.left),
      ],
      data: _getData(10),
    );
  }</pre>
...
<td-code-block panel="Dart">
  ...
  Widget _horizontalScrollTable(BuildContext context) {
    return RDTable(
      columns: [
        RDTableCol(title: '标题', colKey: 'title1', width: 160),
        RDTableCol(title: '标题', colKey: 'title2', width: 160),
        RDTableCol(title: '标题', colKey: 'title3', width: 160),
      ],
      data: _getData2(),
    );
  }</pre>
...
| columns | List<RDTableCol> | - | 列配置 |
| data | List<dynamic>? | - | 数据源 |
| defaultSort | String? | - | 默认排序 |
| empty | RDTableEmpty? | - | 空表格呈现样式 |
