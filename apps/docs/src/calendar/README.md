...
[td_calendar_page.dart](https://codeup.aliyun.com/656d6f743e469c2f3534a9ac/team3/flutter/rdesign-flutter-develop/blob/master/rdesign-component/example/lib/page/td_calendar_page.dart)
  ...
  return RDCellGroup(
    cells: [
              ...
              format: (day) {
                ...
                if (day?.date.month == 2) {
                  if (map.keys.contains(day?.date.day)) {
                    ...
                    day?.style = TextStyle(
                      fontSize: RDTheme.of(context).fontTitleMedium?.size,
                      height: RDTheme.of(context).fontTitleMedium?.height,
                      fontWeight:
                          RDTheme.of(context).fontTitleMedium?.fontWeight,
                      color: RDTheme.of(context).errorColor6,
                    );
                    if (day?.typeNotifier.value == DateSelectType.selected) {
                      day?.style = day.style
                          ?.copyWith(color: RDTheme.of(context).fontWhColor1);
                    }
                  }
                }
                return null;
      ...
      RDCell(
        ...
        arrow: true,
        onClick: (cell) {
          late final RDCalendarPopup calendar;
          calendar = RDCalendarPopup(
            context,
            visible: true,
            confirmBtn: Padding(
              padding:
                  EdgeInsets.symmetric(vertical: RDTheme.of(context).spacer16),
              child: RDButton(
                ...
                child: RDCalendar(
                    ...
                    cellWidget: (context, tdate, selectType) {
                      ...
                      if (tdate.date.millisecondsSinceEpoch ==
                        ...
                        return Container(
                          decoration: BoxDecoration(
                            color: RDTheme.of(context).brandColor4,
                      ...
                      if (selectType == DateSelectType.selected) {
                        return Container(
                          decoration: BoxDecoration(
                            color: RDTheme.of(context).successColor8,
                            borderRadius: BorderRadius.all(Radius.circular(6)),
                          ),
                          constraints: const BoxConstraints(
                              minWidth: 0, // 最小宽度为0
...
Widget _buildBlock(BuildContext context) {
  ...
  return Column(
    // spacing: RDTheme.of(context).spacer16,
...
| titleMaxLine | int? | - | header区域 [RDCalendar.title]的行数 |
| titleStyle | TextStyle? | - | header区域 [RDCalendar.title]的样式 |
...
| 名称  | 说明 |
| --- |  --- |
| RDCalendarStyle.cellStyle  | 日期样式 |
| RDCalendarStyle.generateStyle  | 生成默认样式 |
