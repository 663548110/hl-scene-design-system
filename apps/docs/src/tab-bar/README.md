...
[td_bottom_tab_bar_page.dart](https://codeup.aliyun.com/656d6f743e469c2f3534a9ac/team3/flutter/rdesign-flutter-develop/blob/master/rdesign-component/example/lib/page/td_bottom_tab_bar_page.dart)
...
<td-code-block panel="Dart">
  ...
  Widget _textTypeTabBar(BuildContext context) {
    return RDBottomTabBar(
      RDBottomTabBarBasicType.text,
      useVerticalDivider: false,
      navigationTabs: List.generate(2, (index) {
        final label = '标签${index + 1}';
        return RDBottomTabBarTabConfig(
          tabText: label,
          onTap: () {
...
<td-code-block panel="Dart">
  ...
  Widget _textTypeTabBar3tabs(BuildContext context) {
    return RDBottomTabBar(
      RDBottomTabBarBasicType.text,
      indicatorAnimation: RDBottomTabBarIndicatorAnimation.elastic,
      useVerticalDivider: false,
...
<td-code-block panel="Dart">
  ...
  Widget _iconTextTypeTabBar(BuildContext context) {
    return RDBottomTabBar(
      RDBottomTabBarBasicType.iconText,
      useVerticalDivider: false,
      navigationTabs: List.generate(2, (index) {
        final label = '标签${index + 1}';
        return RDBottomTabBarTabConfig(
...
<td-code-block panel="Dart">
  ...
  Widget _expansionPanelTypeTabBar(BuildContext context) {
    return RDBottomTabBar(
      RDBottomTabBarBasicType.expansionPanel,
      useVerticalDivider: true,
      navigationTabs: [
        RDBottomTabBarTabConfig(
          tabText: '标签',
          onTap: () {
        ...
        RDBottomTabBarTabConfig(
            ...
            popUpButtonConfig: RDBottomTabBarPopUpBtnConfig(
                popUpDialogConfig: RDBottomTabBarPopUpShapeConfig(
                  radius: 10,
                  arrowWidth: 16,
                  arrowHeight: 8,
                ),
                items: ['展开项一', '展开项二', '展开项三']
                    ...
                    .map((e) => PopUpMenuItem(
                          ...
                          itemWidget: Text(
                            e,
                            style: TextStyle(
                                color: RDTheme.of(context).textColorPrimary,
                                fontSize: 16),
                          ),
                        ))
                    .toList(),
                onChanged: (v) {
                  RDToast.showText('点击了 $v', context: context);
                })),
      ],
    );
  }</pre>
...
<td-code-block panel="Dart">
  ...
  Widget _weakSelectTextTabBar(BuildContext context) {
    return RDBottomTabBar(
      RDBottomTabBarBasicType.text,
      componentType: RDBottomTabBarComponentType.normal,
      useVerticalDivider: true,
      navigationTabs: [
        RDBottomTabBarTabConfig(
          badgeConfig: BadgeConfig(
            showBadge: true,
            tdBadge: const RDBadge(RDBadgeType.redPoint),
            badgeTopOffset: -2,
            badgeRightOffset: -10,
          ),
          tabText: '标签',
...
<td-code-block panel="Dart">
  ...
  Widget _capsuleTabBar(BuildContext context) {
    return RDBottomTabBar(RDBottomTabBarBasicType.iconText,
        componentType: RDBottomTabBarComponentType.label,
        outlineType: RDBottomTabBarOutlineType.capsule,
        useVerticalDivider: true,
        navigationTabs: List.generate(3, (index) {
          final label = '标签${index + 1}';
          return RDBottomTabBarTabConfig(
    ...
    return RDBottomTabBar(RDBottomTabBarBasicType.iconText,
        ...
        navigationTabs: List.generate(3, (index) {
          ...
          return RDBottomTabBarTabConfig(
            ...
            onLongPress: () {
              print('长按了${label}');
              RDToast.showText('长按了 $label', context: context);
            },
          );
        }));
  }</pre>
...
| tdBadge | RDBadge? | - | 消息样式（未设置但 showBadge 为 true，则默认使用红点） |
...
| popUpButtonConfig | RDBottomTabBarPopUpBtnConfig? | - | 弹窗配置 |
...
| basicType | RDBottomTabBarBasicType | basicType | 基本样式（纯文本、纯图标、图标+文本） |
| centerDistance | double? | - | icon与文本中间距离（可选） |
| componentType | RDBottomTabBarComponentType? | RDBottomTabBarComponentType.label | 选项样式 默认label |
| currentIndex | int? | - | 选中的index（可选） |
| dividerColor | Color? | - | 分割线颜色（可选） |
| dividerHeight | double? | - | 分割线高度（可选） |
| dividerThickness | double? | - | 分割线厚度（可选） |
| indicatorAnimation | RDBottomTabBarIndicatorAnimation | RDBottomTabBarIndicatorAnimation.none | 指示器动画类型 |
| key |  | - |  |
| navigationTabs | List<RDBottomTabBarTabConfig> | - | tabs配置 |
| needInkWell | bool | false | 是否需要水波纹效果 |
| outlineType | RDBottomTabBarOutlineType? | RDBottomTabBarOutlineType.filled | 标签栏样式 默认filled |
...
| popUpDialogConfig | RDBottomTabBarPopUpShapeConfig? | - | 弹窗UI配置 |

```
```
 ### RDBottomTabBarPopUpShapeConfig
#### 默认构造方法
