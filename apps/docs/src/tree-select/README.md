...
[td_tree_select_page.dart](https://codeup.aliyun.com/656d6f743e469c2f3534a9ac/team3/flutter/rdesign-flutter-develop/blob/master/rdesign-component/example/lib/page/td_tree_select_page.dart)
...
<td-code-block panel="Dart">
  ...
  Widget _buildDefaultTreeSelect(BuildContext context) {
    var options = <RDSelectOption>[];

    for (var i = 1; i <= 10; i++) {
      options.add(RDSelectOption(label: '选项$i', value: i, children: []));

      for (var j = 1; j <= 10; j++) {
        options[i - 1].children.add(RDSelectOption(
              label: '选项$i.$j',
              value: i * 10 + j,
              children: [],
            ));
    ...
    for (var i = 1; i <= 10; i++) {
      ...
      for (var j = 1; j <= 10; j++) {
        options[i - 1].children.add(
            RDSelectOption(label: '选项$i.$j', value: i * 10 + j, children: []));
      }
    }
               ...
               if(mounted) {
                 setState(() {
                   asyncOptions[index].children = [
                     RDSelectOption(label: '异步加载二级-1', value: 101),
                     RDSelectOption(label: '异步加载二级-2', value: 102),
                   ];
                 });
               }
             });
  ...
  Widget _buildThirdTreeSelect(BuildContext context) {
    ...
    for (var i = 1; i <= 3; i++) {
      ...
      for (var j = 1; j <= 3; j++) {
        ...
        for (var k = 1; k <= 3; k++) {
          options[i - 1].children[j - 1].children.add(RDSelectOption(
                label:
...
| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| children | List<RDSelectOption> | const [] | 子选项 |
...
| onChange | RDTreeSelectChangeEvent? | - | 选中值发生变化 |
| options | List<RDSelectOption> | const [] | 展示的选项列表 |
| style | RDTreeSelectStyle | RDTreeSelectStyle.normal | 一级菜单样式 |
